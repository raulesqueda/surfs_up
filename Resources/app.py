...
@app.route("/superhero_name/<real_name>")
def ep_superhero_namee(real_name):
    found = False 
    for sh in justice_league_members:
    if sh["real_name"]==real_name:
        superheroe_name=sh["superhero"]
        found=True
    if found:
        return f"Superhero name: {superheroe_name"}
    else:
